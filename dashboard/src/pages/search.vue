        ...
        <div class="home">
          <el-row id="posts" type="flex" justify="space-around">
            <el-col :span="16">
              ...
              <h2 v-else style="text-align: center">没有找到有关内容。</h2>
            </el-col>
...
export default {
  ...
  methods: {
    // 获取文章列表
    async getArtList() {
      const { data: res } = await this.$axios.get('http://localhost:8080/searchPassage', {
        params: {
          searchKey: this.$route.params && this.$route.params.word
        }
      }).then(response => {
