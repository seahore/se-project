...
export default {
  ...
  mounted() {
    ...
    this.following = this.$axios.get('http://localhost:8080/selectFollowUser')
    this.getArtList()
  },
  methods: {
    // 获取文章列表
    async getArtList() {
      const { data: res } = await this.$axios.get('http://localhost:8080/ad/showPassagesAd').then(response => {
        response.data.forEach(p => {
          if (!this.following.includes(p.userId)) { return }
